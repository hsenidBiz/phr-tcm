...
fn the_page_embeds_the_revision_it_is_written_at() {
    ...
    // note_server's test bumps this counter; a bump between the render and
    // the read below would put the two one apart.
    let _revisions = crate::serial::report_revisions();
    let html = draft_page(&[draft("A", None, "")], vec![String::new()], vec![]);
