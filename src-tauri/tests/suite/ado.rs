...
async fn maps_429_with_retry_after() {
    // The 429 sets the process-wide hold (5 s). Clear it when this test
    // ends, pass or fail - the reset throttle_backoff.rs uses - or every
    // other test in this binary waits it out in `pace()`. The pacer lock
    // keeps throttle_backoff's own holds (and its clears) out of this one.
    //
    // Even cleared at once, any request another test sends in the moment
    // between the 429 and the clear sleeps the whole hold - and with every
    // module in one binary there are enough of those that it happened in
    // about one run in three. 5 s is still well past the 2 s check below,
    // and short enough to finish inside the rest of the suite; it was 17.
    ...
    let err = client.get_projects("o").await.unwrap_err();
    assert!(matches!(err, AdoError::RateLimited { retry_after_secs: 5 }));

    drop(clear_hold);
    let t = std::time::Instant::now();
    v2_lib::ado::throttle::pace().await;
    assert!(t.elapsed() < std::time::Duration::from_secs(2), "the 5 s hold must not outlive this test");
}
