...
/// other's holds. The suite's pacer lock serialises these tests with every
/// other module's that sets a hold or times a request; each also clears
/// the shared state on entry and exit.
struct Gate {
    level_ms: u64,
    _pacer: std::sync::MutexGuard<'static, ()>,
}

fn gate() -> Gate {
    let pacer = crate::serial::pacer();
    Gate { level_ms: throttle::current_interval_ms(), _pacer: pacer }
}

/// Puts back the level the test found, pass or fail, before the lock is
/// released: `throttle_default` reads the STARTING level in this same
/// process, and `reset()`'s "full" left behind would let a changed default
/// pass there unnoticed.
impl Drop for Gate {
    fn drop(&mut self) {
        throttle::clear_backoff();
        if let Some(level) = ["full", "balanced", "gentle"]
            .into_iter()
            .find(|l| throttle::interval_for(l) == self.level_ms)
        {
            throttle::set_level(level);
        }
    }
}
...
/// This suite is `AdoClient` exercised with no Tauri app anywhere in it -
...
/// The permission answers the delete gate needs (see tests/suite/deletion.rs).
