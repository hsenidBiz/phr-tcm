...
/// Resolve what the user picked in the DB-server browse into something an
/// MCP client can actually LAUNCH. Registration used to write the picked
/// path verbatim; picking the repo folder registered a directory as the
/// command, the client's spawn failed silently, and every tool try died
/// before the server existed. Never again: only a real invocation leaves
/// this function.
///
///   .exe/.cmd/.bat  -> run it directly
///   .dll            -> `dotnet <dll>` (a published framework-dependent build)
///   directory       -> the newest built .exe beneath it (bin output;
///                      obj/ and *Tests* builds excluded) - or a clear
///                      "run `dotnet build` first" refusal when none exists
    ...
    while let Some(dir) = stack.pop() {
        ...
        for entry in entries.flatten() {
            ...
            if p.is_dir() {
                ...
                if name == "obj" || name == "ref" || name.contains("test") || name == "node_modules" || name.starts_with('.') {
                    continue;
                }
                stack.push(p);
