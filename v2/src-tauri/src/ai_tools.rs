...
pub const COMMANDS: &[CommandSpec] = &[
    CommandSpec {
        ...
        body: &[
            ...
            "where the JSON goes, which specs are authoritative, what is out of scope. Those",
            "are theirs to answer, not yours to assume. Answering the file-location one makes",
            "the app watch that path, so what you write imports itself.",
            "",
    ...
    CommandSpec {
        ...
        body: &[
            ...
            "the plan is approved. Take the slice list from `check_spec_coverage`'s section",
            "inventory, or from the intake plan's own scale block if coverage has not run yet -",
            ...
            "Run `optimize_cases` exactly once, on the merged file, never per slice. Then run",
            "`check_spec_coverage` on that merged file and account for every `uncovered` entry",
            "out loud before handing it over.",
        ],
    },
    CommandSpec {
        stem: "guide",
