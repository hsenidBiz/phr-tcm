...
/// The Claude Code skill written alongside the MCP registration.
///
/// # Why a skill, and why this thin one
///
/// Without it the tools are only reachable by name: somebody has to know
/// `begin_test_case_writing` exists and type it. A skill's `description` is
/// what an assistant matches against, so naming the situation - "test cases
/// for a PBI" - is what makes the tools get picked up from an ordinary
/// request.
///
...
/// silently. So the skill is a trigger and a pointer, and the guide stays
/// the single source.
pub const SKILL_MARKER: &str = "<!-- generated by Test Case Manager -->";

/// `~/.claude/skills/tcm-testcases/SKILL.md`.
pub fn skill_path(home: &str) -> PathBuf {
    PathBuf::from(home)
        .join(".claude")
        .join("skills")
        .join(TCM_SERVER)
...
pub fn skill_markdown() -> String {
    ...
    // in the YAML frontmatter - a folded scalar whose continuation lines
    // are at column 0 is not valid YAML, and the description is the whole
    // reason the skill exists.
    //
    // Hence also the single-line description: nothing to indent, nothing to
    // get wrong.
    let lines: Vec<String> = vec![
        "---".into(),
        format!("name: {TCM_SERVER}"),
        "description: Write, review or bulk-edit Azure DevOps test cases through Test Case \
Manager. Use when asked to write test cases for a PBI, backlog item or user story, to update \
existing cases in bulk, to check a draft of test cases, or when Test Case Manager is mentioned. \
Also use before hand-writing test-case JSON, because the tools carry the project's live modules, \
tags and validation."
            .into(),
        "---".into(),
        String::new(),
        SKILL_MARKER.into(),
        String::new(),
        "# Test Case Manager".into(),
        String::new(),
        format!(
            "The `{TCM_SERVER}` MCP server is connected to a running copy of Test Case Manager, \
signed in to a real Azure DevOps organization."
        ),
        String::new(),
