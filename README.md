...
manifest quirk crashes tauri-linked unit-test binaries. They are one
binary, `src-tauri/tests/suite/` (one module per file, listed in its
`main.rs`), because every separate test binary is another full link of
the crate and Tauri. Add a test file there, not directly under `tests/`.
Run one module with `cargo test --test suite <module>::`.
...
src-tauri/                  Rust core
  ...
  tests/suite/              Integration tests, one binary (wiremock and
                            golden vectors)
  tests/bindings.rs         Generates src/bindings.ts
